...
import static com.google.common.collect.Strea
...
*//**
 * A {@link org.springframework.http.converter.HttpMessageConverter} capable of
 * generating and parsing columnar files with single-character delimiters, such
 * as CSV and TSV files.
 *//*
public final class DelimiterSeparatedValueHttpMessageConverter extends AbstractGenericHttpMessageConverter<Object> {
	...
	*//**
	 ...
	 *            The list of {@link MediaType}s that should be supported by
	 *            this converter.
	 * @param delimiter
	 *            The character to be used as value delimiter.
	 * @param includeHeader
	 *            Flag indicating whether a header is expected to be read or
	 *            written.
	 * @param csvMapper
	 *            The {@link CsvMapper} to which the heavy lifting is delegated.
	 *//*
	private DelimiterSeparatedValueHttpMessageConverter(final ImmutableSet<MediaType> supportedMediaTypes,
			final char delimiter, final boolean includeHeader, final CsvMapper csvMapper) {
	...
	*//**
	 * Creates a default TSV converter supporting the
	 * {@link MediaTypes#TEXT_TSV} media type.
	 *
	 * @param csvMapper
	 *            {@link CsvMapper} used for de-/serialization.
	 * @return A non-{@code null}
	 *         {@link DelimiterSeparatedValueHttpMessageConverter} configured
	 *         for TSV files.
	 *//*
	public static DelimiterSeparatedValueHttpMessageConverter tsv(final CsvMapper csvMapper) {		
		return new DelimiterSeparatedValueHttpMessageConverter(ImmutableSet.of("TEXT/CSV"), '\t', true,csvMapper);
	}

	*//**
	 * {@inheritDoc}
	 *
	 * @implNote We override this method because the super implementation
	 *           provides the <em>context class</em> to the {@link #supports}
	 *           method. That seems "just plain wrong": the type of the context
	 *           class is irrelevant.
	 *           <p>
	 * 			Moreover, we only allow reading super types of
	 *           {@link ImmutableList}, since {@link #read} always returns an
	 *           {@link ImmutableList}.
	 *//*
	@Override
	...
	@Override
	public ImmutableList<?> read(final Type type, final Class<?> contextClazz, final HttpInputMessage inputMessage)
			throws IOException {
		return ImmutableList.copyOf(getReader(getElementType(type)).readValues(getInputReader(inputMessage)));
	}

	@Override
	protected ImmutableList<?> readInternal(final Class<?> clazz, final HttpInputMessage inputMessage)
			throws IOException {
		return ImmutableList.copyOf(getReader(getElementType(clazz)).readValues(getInputReader(inputMessage)));
	}

	*//**
	 * Returns a {@link Reader} containing the contents of the
	 * {@link HttpInputMessage}.
	 *
	 * @param inputMessage
	 *            the {@link HttpInputMessage} of which the content should be
	 *            read.
	...
	private Reader getInputReader(final HttpInputMessage inputMessage) throws IOException {
		 ...
		 * Respect the charset if specified in the request headers; default to
		 * UTF-8 if absent. (Note that we ignore the HTTP 1.1 spec here, which
		 * specifies that ISO-8859-1 is the default charset.)
		 
		final Charset contentCharSet = Optional.ofNullable(inputMessage.getHeaders().getContentType())
	...
	*//**
	 ...
	 *
	 * @param clazz
	 *            The type of instances to deserialize.
	...
	@Override
	protected void writeInternal(final Object instance, final Type type, final HttpOutputMessage outputMessage)
			throws IOException {
		try (SequenceWriter writer = getWriter(getElementType(type)).writeValues(getOutputWriter(outputMessage));
	...
	*//**
	 * Returns a {@link Writer} for producing the contents of the given
	 * {@link HttpOutputMessage}.
	 *
	 * @param outputMessage
	 *            The {@link HttpOutputMessage} to which the content should be
	 *            written.
	 * @return A non-{@code null} {@link Writer}.
	 * @throws IOException
	 *             If the response body cannot be written to.
	 *//*
	private Writer getOutputWriter(final HttpOutputMessage outputMessage) throws IOException {
		
		 * Respect the charset using the Content-Type header, if any; default to
		 * ISO-8859-1 otherwise.
		 
		final Charset outputCharset = Optional.ofNullable(outputMessage.getHeaders().getContentType())
				.map(MediaType::getCharset).orElse(StandardCharsets.ISO_8859_1);
	...
	*//**
	 ...
	 *
	 * @param clazz
	 *            The type of instances to serialize.
	...
	*//**
	 ...
	 *
	 * @param clazz
	 *            The type to be read or written.
	...
	*//**
	 * Returns the element type captured by the given {@link ImmutableList}
	 * class.
	 *
	 * @param type
	 *            The list type.
	...
	private Class<?> getElementType(final Type type) {
		...
		ParameterizedType tableType = (ParameterizedType) TypeToken.of(type).getSupertype((Class) superType).getType();
	...
	*//**
	 ...
	 *
	 * @param instance
	 *            The object that should be converted to a stream.
